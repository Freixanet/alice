...
struct SystemExtrasScreen: View {
    ...
    }.navigationTitle("Advanced Operations").navigationBarTitleDisplayMode(.inline).scrollContentBackground(.hidden).background(Palette.background(scheme)).task{await loadProfiles();await load()}.task(id:activeAction?.name){await pollAction()}.onChange(of:profile){_,_ in Task{await loadComputer()}}.refreshable{await load()}
