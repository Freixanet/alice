...
private struct PluginInstallSheet: View {
    ...
    private func run() async { busy=true; defer{busy=false}; do{try await install(identifier.trimmingCharacters(in:.whitespacesAndNewlines),force,enable);dismiss()}catch{failure=(error as? LocalizedError)?.errorDescription ?? "Install failed."} }
}
