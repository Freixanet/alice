...
async function createPgliteSql(): Promise<Sql> {
  ...
  // One instance per process, shared across HMR. On this Mac it lives in
  // ~/.alice/pglite so accounts survive `npm run dev` restarts.
  globalRef.__pgliteDiskInstance__ ??= (async () => {
    const { PGlite } = await import("@electric-sql/pglite");
    const { mkdirSync } = await import("node:fs");
    const { homedir } = await import("node:os");
    const { join } = await import("node:path");
    // Persist accounts across `npm run dev` restarts so guests keep their login.
    // Vercel / serverless stays in-memory — set DATABASE_URL (Neon) there.
    const fromEnv = process.env.ALICE_PGLITE_DIR?.trim();
    const dataDir =
      process.env.VERCEL || process.env.ALICE_PGLITE_MEMORY === "1"
        ? undefined
        : fromEnv || join(homedir(), ".alice", "pglite");
  ...
  // files are tracked in _migrations. The glob does not descend. Runs once per
  // module instance
 ...
 *
 * - **PGLite** (no `DATABASE_URL`): open ~/.alice/pglite and apply
