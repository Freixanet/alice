...
export { GROK_PROVIDERS, LOGIN_SOCIAL };
...
/**
 * Google / Apple on `/login`. Prefers this app's native Better Auth social
 * providers (env credentials). Live preview, and local when those env vars are
 * missing, federates through the Grok broker (`GROK_PROVIDERS`).
 */
export async function signInWithSocial(
  ...
  const brokerId = LOGIN_SOCIAL.find((p) => p.id === provider)?.brokerId;
  if (!brokerId) throw new Error("Unknown provider");

  if (inLivePreview()) {
    await signIn(brokerId, { callbackURL, errorCallbackURL });
    return;
  }
