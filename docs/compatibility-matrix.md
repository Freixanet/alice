...
| Agents / canonical sessions | Native; New Agent uses an independent profile session; create/rename share the plugin engine; reuse needs the job that minted the profile; directory identity change is refused until Hermes can coordinate it outside the moved profile | Companion controls               | Dashboard RPC, plugin `/agents`, and profile identity                           |
