...
identified by their personal-assistant sites; the user's spelling "Caddu" is
provisionally mapped to Cadu, the Hermes iPhone client. There are multiple OpenMuse
projects, so both relevant implementations are recorded instead of conflated.
...
| [Persona](https://persona.chat/)                                | Calendar, email drafts, contextual location, memory, bookings and outbound calls; wearable advertised                                   | Location triggers, memory and calendar exist. Calls and a wearable are separate integrations/hardware, not native features Alice can claim                                                           |
...
| [Caddy](https://caddy.app/)                                     | Personal assistant in messages, reminders, calendar, notes and contextual follow-ups                                                    | Similar building blocks exist through Hermes; delivery, setup simplicity and task outcomes need comparative testing                                                                                  |
