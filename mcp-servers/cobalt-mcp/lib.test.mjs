...
test("safeFileName keeps a readable name and strips what a filesystem or a label rejects", () => {
  ...
  assert.equal(
    safeFileName('a/b\\c:d*e?f"g<h>i|j[k]l.mp4'),
    "a_b_c_d_e_f_g_h_i_j_k_l.mp4",
  );
...
test("uniquePath never overwrites an earlier download", () => {
  ...
  assert.equal(
    uniquePath("/d", "clip.mp4", exists),
    path.join("/d", "clip (3).mp4"),
  );
  assert.equal(
    uniquePath("/d", "fresh.mp4", exists),
    path.join("/d", "fresh.mp4"),
  );
  assert.equal(
    uniquePath("/d", "noext", (p) => p === "/d/noext"),
    path.join("/d", "noext (2)"),
  );
});
...
test("encodeForLink encodes the characters that break a markdown link", () => {
  ...
  assert.equal(
    decodeURIComponent(encodeForLink("🚀 ¿Cuántas？.mp4")),
    "🚀 ¿Cuántas？.mp4",
  );
});
...
test("mediaMarkdown is one line Alice parses back to the same path and mirror", () => {
  const dest =
    "/Users/m/.hermes/profiles/descargas/workspace/descargas/Pole (1080p, h264).mp4";
  const mirror =
    "http://MacBook-Pro-de-Marcos.local:9000/tunnel?id=6skTcd&exp=1&sig=HFpW";
  const line = mediaMarkdown(dest, mirror);
  assert.match(
    line,
    /^!\[Pole \(1080p, h264\)\.mp4\]\(alice:\/\/file\?path=.+&url=.+\)$/,
  );
...
test("extensionFor prefers the URL, then the content type, then the picker type", () => {
  ...
  assert.equal(
    extensionFor({
      url: "https://cdn/tunnel?id=1",
      contentType: "video/mp4; charset=x",
    }),
    ".mp4",
  );
  assert.equal(
    extensionFor({ url: "https://cdn/tunnel", type: "photo" }),
    ".jpg",
  );
...
test("describeCobaltError speaks for the common codes and never crashes on unknown ones", () => {
  assert.equal(
    describeCobaltError("error.api.content.video.private"),
    "That video is private.",
  );
  assert.match(
    describeCobaltError("error.api.fetch.fail"),
  ...
  assert.match(
    describeCobaltError("error.api.content.too_long", { limit: 10800 }),
    /\(limit=10800\)$/,
  );
});
...
test("savedReport lists the fields the SOUL reads, media_markdown included", () => {
  ...
  assert.ok(
    lines[4].startsWith("media_markdown: ![clip.mp4](alice://file?path="),
  );
